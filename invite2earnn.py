...
def set_webhook():
    try:
        bot.remove_webhook()
        time.sleep(1)
        bot.set_wfrom flask import Flask, request
from threading import Thread
import telebot
from telebot import types
import sqlite3
...
if __name__ == '__main__':
    try:
        ...
        logging.error(f"خطأ رئيسي: {e}")ebhook(url=f'https://invite2earnn.onrender.com/7897542906:AAFWO23YZhUhLpDJ500d6yZ4jcUnPZY450g')
        return "تم إعداد Webhook بنجاح!", 200
    except Exception as e:
        logging.error(f"خطأ في إعداد webhook: {e}")
        return "فشل إعداد Webhook", 500

if __name__ =from flask import Flask, request
from threading import Thread
import telebot
from telebot import types
import sqlite3
...
if __name__ == '__main__':
    try:
        ...
        logging.error(f"خطأ رئيسي: {e}")= '__main__':
    try:
        print("🚀 البوت يعمل الآن...")
        bot.remove_webhook()
        time.sleep(1)
